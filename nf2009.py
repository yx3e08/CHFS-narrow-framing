...
def optimal_w3(Gamma, W2, Beta = .98, loss_aversion = 2.25, B0 = 0.01, gm = .88, delta = 1, first_guess = 0.025):
    gamma, beta, LA, b0, gm, delta, w2 = Gamma, Beta, loss_aversion, B0, gm, delta, W2
...
def b_search(self_Gamma = 4.0, self_W2 = 0, target_w = 0.2):
    b = 0.1
    w = round(optimal_w3(Gamma = self_Gamma, B0= b, W2 = self_W2), 2)
    k = 0
    while True:
        print (w)
        if abs(w - target_w) >= 0.3:
            if w > target_w:
                b += 0.2
                w = round(optimal_w3(Gamma = self_Gamma, B0= b, W2 = self_W2), 2)
                k += 1
        ...
        elif 0.000001 < abs(w - target_w) < 0.15:
            if w > target_w:
                    b += 0.001
                    w = round(optimal_w3(Gamma = self_Gamma, B0= b, W2 = self_W2), 2)
                    k += 1
...
# print (b)
#w_list.append(optimal_w3(Gamma = 3.0, B0 = b, W2 = 0.5))
